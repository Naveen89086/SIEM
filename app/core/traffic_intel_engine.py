...
class TrafficIntelligenceEngine:
    ...
    def __init__(self, window_seconds=60):
        ...
        self.malicious_ips = {"1.1.1.1", "8.8.8.8"}
        self.alerts = []
    ...
    def get_aggregated_metrics(self):
        ...
        for proto, data in aggr.items():
            ...
            threat_level = "NORMAL"
            if entropy > 4.5 or len(data["ports"]) > 20: threat_level = "ANOMALY"
            if any(ip in self.malicious_ips for ip in data["src_ips"]): threat_level = "THREAT"
            if pps > 500: threat_level = "THREAT"
            variance = round((len(data["dst_ips"]) / data["packet_count"] * 100), 1) if data["packet_count"] > 0 else 0
            results.append({
                "protocol": proto, "packet_count": data["packet_count"],
                "avg_size": round(data["byte_count"] / data["packet_count"], 1) if data["packet_count"] > 0 else 0,
                "pps": pps, "bps": bps, "unique_src": len(data["src_ips"]), "unique_dst": len(data["dst_ips"]),
                "entropy": entropy, "traffic_type": traffic_type, "threat_level": threat_level, "variance": variance
            })
